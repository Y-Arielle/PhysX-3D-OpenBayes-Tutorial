    ...
    def finetune_from(self, finetune_ckpt):
        ...
        for name, model in self.models.items():
            ...
            if name in finetune_ckpt:
                ...
                model_ckpt = load_file(finetune_ckpt[name])#torch.load(read_file_dist(finetune_ckpt[name]), map_location=self.device, weights_only=True)
                for k, v in model_ckpt.items():
                    model_ckpt[k]=model_ckpt[k].to(device=self.device)
