...
class SparseFlowMatchingTrainer(FlowMatchingTrainer):
    ...
    def training_losses(
        ...
        **kwargs
        ...
        noise_phy = x_0_phy.replace(torch.randn_like(x_0_phy.feats))
        x_t_phy = self.diffuse(x_0_phy, t, noise=noise)

        
        
        pred = self.training_models['denoiser'](x_t, t * 1000, None,cond, **kwargs)
        cond_aes=self._v_to_xstart(x_t,t,pred)
        pred_phy = self.training_models['denoiser_phy'](x_t_phy, t * 1000, cond_aes, cond,**kwargs)
        assert pred.shape == noise.shape == x_0.shape
        target = self.get_v(x_0, noise, t)
        target_phy = self.get_v(x_0_phy, noise_phy, t)

        terms = edict()
        terms["mse"] = F.mse_loss(pred.feats, target.feats)
        terms["mse_phy"] = F.mse_loss(pred_phy.feats, target_phy.feats)
        terms["loss"] = terms["mse"]+terms["mse_phy"]
        ...
        mse_per_instance_phy = np.array([
            F.mse_loss(pred_phy.feats[x_0_phy.layout[i]], target_phy.feats[x_0_phy.layout[i]]).item()
            for i in range(x_0_phy.shape[0])
        ])
        #time_bin = np.digitize(t.cpu().numpy(), np.linspace(0, 1, 11)) - 1
        for i in range(10):
            if (time_bin == i).sum() != 0:
                terms[f"bin_{i}"] = {"mse": mse_per_instance_phy[time_bin == i].mean()}
    ...
    def run_snapshot(
        ...
        for i in range(0, num_samples, batch_size):
            ...
            noise_phy = data['x_0_phy'].replace(torch.randn_like(data['x_0_phy'].feats))
            sample_gt_phy.append(data['x_0_phy'])
            del data['x_0_phy']
            ...
            res_phy = sampler.sample(
                self.models['denoiser_phy'],
                noise=noise_phy,
                cond_aes=res.samples,
                **args,
                steps=50, cfg_strength=3.0, verbose=verbose,
            )
            sample_phy.append(res_phy.samples)



        sample_gt = sp.sparse_cat(sample_gt)
        sample_gt_phy = sp.sparse_cat(sample_gt_phy)
        sample = sp.sparse_cat(sample)
        sample_phy = sp.sparse_cat(sample_phy)
        sample_dict = {
            'sample_gt': {'value': {'x_0':sample_gt,'x_0_phy':sample_gt_phy}, 'type': 'sample'},
            'sample': {'value': {'x_0':sample,'x_0_phy':sample_phy}, 'type': 'sample'},
        }
...
class ImageConditionedSparseFlowMatchingCFGTrainerphy(ImageConditionedMixin, SparseFlowMatchingCFGTrainer):
    """
