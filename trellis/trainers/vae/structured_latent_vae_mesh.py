...
class SLatVaeMeshTrainer(BasicTrainer):
    """
    ...
    def geometry_losses(
        ...
        with torch.no_grad():
            ...
            for i in range(len(reps)):
                gtphy=torch.cat([mesh[i]['property1'].reshape(len(mesh[i]['property1']),-1),mesh[i]['property2']],1)
                gt_mesh = MeshExtractResult(mesh[i]['vertices'].to(self.device), mesh[i]['faces'].to(self.device),phy_property=gtphy.to(self.device))
                gt_meshes.append(gt_mesh)
            target = self._render_batch(gt_meshes, extrinsics, intrinsics, return_types=['mask', 'depth', 'normal','lang_map_all','phy_map_all']) #'phy_map' ,'lang_map'
        ...
        return_types = ['mask', 'depth', 'normal','lang_map', 'phy_map'] if self.use_color else ['mask', 'depth', 'normal']
        ...
        lang_map,phy_map=self.training_models['property_output'](buffer['lang_map'],buffer['phy_map'])
        ...
        if success_mask.sum() != 0:
            ...
            b,_,c,w,h=target['lang_map_all'].shape
            terms['lang_loss'] = l1_loss((lang_map.reshape(b,4,-1,w,h)* target['mask'])[:,:3,:], (target['lang_map_all']* target['mask'])[:,:3,:]) 
            target['phy_map_all'][:,0]=(target['phy_map_all'][:,0]-95)/48
            target['phy_map_all'][:,1]=1-target['phy_map_all'][:,1]/10
            target['phy_map_all'][:,2]=(target['phy_map_all'][:,2]-2.3)/2.8
            target['phy_map_all'][:,4]=(target['phy_map_all'][:,4]+0.812)/0.732

            target['phy_map_all'][:,-1]=target['phy_map_all'][:,-1]/4
            terms['phy_loss'] = l1_loss(phy_map* target['mask'], target['phy_map_all']* target['mask']) 

            #imageio.imwrite('a.png',np.uint8(target['phy_map'][0,1]*128))

            terms['allphy_loss'] = terms['phy_loss']+terms['lang_loss']
    ...
    def training_losses(
        ...
        alpha: torch.Tensor,
        mesh: List[Dict],
        #latents_phy: SparseTensor,
        property1: SparseTensor,
        property2: SparseTensor,
        extrinsics: torch.Tensor,
        intrinsics: torch.Tensor,
        ...
        """
        z, mean, logvar = self.training_models['property_encoder'](property1,property2, sample_posterior=True, return_raw=True)
        phy_property,hs = self.training_models['property_decoder'](z)
        reps = self.training_models['decoder'](latents,phy_property,hs)
        ...
        terms['loss'] = terms['loss'] + terms['geo_loss'] + terms['allphy_loss']
    ...
    def run_snapshot(
        ...
        for i in range(0, num_samples, batch_size):
            ...
            z = self.models['property_encoder'](args['property1'],args['property2'], sample_posterior=True, return_raw=False)
            phy_property,hs = self.models['property_decoder'](z)
            rep=self.models['decoder'](args['latents'],phy_property,hs)
            reps.extend(rep)
        ...
        gt_render_results = self._render_batch([
            MeshExtractResult(vertices=mesh['vertices'].to(self.device), faces=mesh['faces'].to(self.device), phy_property=torch.cat([mesh['property1'].reshape(len(mesh['property1']),-1),mesh['property2']],1).to(self.device))
            for mesh in gt_meshes
        ], exts, ints, return_types=['normal','phy_map_all'])
        ret_dict.update({f'gt_normal': {'value': self._flip_normal(gt_render_results['normal'], exts, ints), 'type': 'image'}})
        
     
        ret_dict.update({f'gt_phy': {'value': (gt_render_results['phy_map_all'][:,1]==1).float()[:,None].repeat(1,3,1,1), 'type': 'image'}})
        return_types = ['normal','phy_map']
        if self.use_color:
        ...
        lang_map,phy_map=self.models['property_output'](None,render_results['phy_map'])
        render_results['phy_map']=phy_map

        ret_dict.update({f'rec_phy': {'value': ((torch.relu(render_results['phy_map'][:,1])>0.85)&(torch.relu(render_results['phy_map'][:,1])<1)).float()[:,None].repeat(1,3,1,1), 'type': 'image'}})
