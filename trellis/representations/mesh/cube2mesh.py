...
class MeshExtractResult:
    def __init__(self,
        ...
        self.res = res
        self.phy_property = phy_property
...
class SparseFeatures2Mesh:
    ...
    def __call__(self, cubefeats : SparseTensor, phy_property=None, training=False):
        """
        ...
        if phy_property is not None:
            ...
            phy=phy_property.feats.reshape(len(sdf),8,-1)


            sdf += self.sdf_bias
            v_attrs = [sdf, deform, color, phy,] if self.use_color else [sdf, deform]
            ...
            if self.use_color:
                sdf_d, deform_d, colors_d,phy = v_attrs_d[..., 0], v_attrs_d[..., 1:4], v_attrs_d[..., 4:10], v_attrs_d[..., 10:]
            else:
                sdf_d, deform_d = v_attrs_d[..., 0], v_attrs_d[..., 1:4]
                colors_d = None
                
            x_nx3 = get_defomed_verts(self.reg_v, deform_d, self.res)
            vertices, faces, L_dev, veratt = self.mesh_extractor(
                ...
                voxelgrid_colors=torch.cat([colors_d,phy],1),
                training=training)
            mesh = MeshExtractResult(vertices=vertices, faces=faces, vertex_attrs=veratt[:,:6], res=self.res, phy_property=veratt[:,6:])
            if training:
                if mesh.success:
