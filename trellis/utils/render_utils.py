...
def render_frames(sample, extrinsics, intrinsics, options={}, colors_overwrite=None, verbose=True,return_types=["color","mask", "normal","allphy_map"], **kwargs):
...
def render_video(sample, resolution=512, bg_color=(0, 0, 0), num_frames=300, r=2, fov=40, **kwargs):
    ...
    return render_frames(sample, extrinsics, intrinsics, {'resolution': resolution, 'bg_color': bg_color}, return_types=["color","mask", "normal","allphy_map","rendervis"],**kwargs)

def render_video_gt(gt, resolution=512, bg_color=(0, 0, 0), num_frames=300, r=2, fov=40, **kwargs):
    yaws = torch.linspace(0, 2 * 3.1415, num_frames)
    pitch = 0.25 + 0.5 * torch.sin(torch.linspace(0, 2 * 3.1415, num_frames))
    yaws = yaws.tolist()
    pitch = pitch.tolist()
    ...
    gtmesh=MeshExtractResult(vertices=torch.Tensor(gt[0]).cuda(), faces=torch.Tensor(np.int32(gt[1])).cuda(), phy_property=torch.from_numpy(gt[2][:,None]).cuda())
    return render_frames(gtmesh, extrinsics, intrinsics, {'resolution': resolution, 'bg_color': bg_color}, return_types=["mask", "allphy_map",'normal'],**kwargs)


def render_video_gt_onlycolor(gt, resolution=512, bg_color=(0, 0, 0), num_frames=300, r=2, fov=40, **kwargs):
    yaws = torch.linspace(0, 2 * 3.1415, num_frames)
    pitch = 0.25 + 0.5 * torch.sin(torch.linspace(0, 2 * 3.1415, num_frames))
    yaws = yaws.tolist()
    pitch = pitch.tolist()
    ...
    gtmesh=MeshExtractResult(vertices=torch.Tensor(gt[0]).cuda(), faces=torch.Tensor(np.int32(gt[1])).cuda(), vertex_attrs=gt[2])
    return render_frames(gtmesh, extrinsics, intrinsics, {'resolution': resolution, 'bg_color': bg_color}, return_types=["color_gt",'normal'],**kwargs)
