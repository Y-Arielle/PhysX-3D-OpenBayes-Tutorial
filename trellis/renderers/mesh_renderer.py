    ...
    def render(
        ...
        for type in return_types:
            ...
            if type == "mask" :
                ...
                img=[]
                lang=mesh.phy_property[:,:16][:,None,:]
                
                img = dr.interpolate(lang.permute(1,0,2).float().contiguous(), rast, faces_int)[0]

            elif type == "phy_map" :
                phy=mesh.phy_property[:,-16:][:,None,:]
                img = dr.interpolate(phy.permute(1,0,2).float().contiguous(), rast, faces_int)[0]
            
            elif type == "allphy_map" :
                phy=mesh.phy_property[:,None,:]
                img = dr.interpolate(phy.permute(1,0,2).float().contiguous(), rast, faces_int)[0]
                

            elif type == "lang_map_all" :
                img=[]
                lang=mesh.phy_property[:,:3072].reshape(-1,4,768)
                
                img = dr.interpolate(lang.permute(1,0,2).float().contiguous(), rast.repeat(4,1,1,1), faces_int)[0]
            elif type == "gtallphy_map" :
                
                phy=mesh.phy_property[:,None,:].clone()
                img = dr.interpolate(phy.permute(1,0,2).float().contiguous(), rast, faces_int)[0]
            elif type == "rendervis" :

                rendervis=mesh.render_vis[:,None,:]
                img = dr.interpolate(rendervis.permute(1,0,2).float().contiguous(), rast, faces_int)[0]
            elif type == "phy_map_all" :
                phy=mesh.phy_property[:,-14:][:,None,:]
                img = dr.interpolate(phy.permute(1,0,2).float().contiguous(), rast, faces_int)[0]
                ...
                texc, _ = dr.interpolate(mesh.vertex_attrs[0][None, ...].float().cuda(), rast, faces_int.int())
                color = dr.texture(mesh.vertex_attrs[1][None, ...].float().cuda(), texc, filter_mode='linear')
                img = color * torch.clamp(rast[..., -1:], 0, 1)
